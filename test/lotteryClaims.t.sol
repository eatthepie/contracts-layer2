...
contract LotteryClaimsTest is Test {
    ...
    // Claim periods
    function testClaimPrizeJustBeforeClaimPeriodEnds() public {
        // Setup initial game and buy a winning ticket
        fundLottery(5000);
        ...
        setupDrawAndVDF();
        lottery.setWinningNumbersForTesting(initialGameNumber, winningTicket[0]);
        lottery.calculatePayouts(initialGameNumber);

        // Play N-1 games (where N is CLAIM_PERIOD_GAMES)
        vm.pauseGasMetering();
        for (uint256 i = 0; i < lottery.CLAIM_PERIOD_GAMES() - 1; i++) {
            fundLottery(5000);
            uint256 nextGameNumber = lottery.currentGameNumber();
            setupDrawAndVDF();
            lottery.calculatePayouts(nextGameNumber);
        }
        ...
        uint256 initialBalance = player1.balance;
        vm.prank(player1);
        lottery.claimPrize(initialGameNumber);

        // Assert
        assertTrue(player1.balance > initialBalance, "Player should have received a prize");
        assertTrue(lottery.prizesClaimed(initialGameNumber, player1), "Prize should be marked as claimed");
    }

    function testClaimPrizeAfterClaimPeriod() public {
        // Setup initial game and buy a winning ticket
        fundLottery(5000);
        uint256 initialGameNumber = lottery.currentGameNumber();
        uint256[4][] memory winningTicket = new uint256[4][](1);
        ...
        // Attempt to claim the prize (should fail)
        uint256 initialBalance = player1.balance;
        vm.prank(player1);
        vm.expectRevert("Claim period has ended");
        lottery.claimPrize(initialGameNumber);

        // Assert
        assertEq(player1.balance, initialBalance, "Player balance should not have changed");
        assertFalse(lottery.prizesClaimed(initialGameNumber, player1), "Prize should not be marked as claimed");
    }
